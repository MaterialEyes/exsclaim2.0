...
- `src`: Majority of relevant source code
  - `components`: This is where react components go. They are divided between `common` for components on every page, `results` for components relating to viewing results, and `search` for components relating to submitting new exsclaim queries. 
