...
async def launch(args=None):
	...
	group.add_argument("-idb", "--initialize_db", action="store_true",
					   help="Initializes the PostgreSQL database from scratch using the schema generated by the current models. Cannot be used with update_db.")
	group.add_argument("-udb", "--update_db", action="store_true",
					   help="Uses Alembic to upgrade the existing database schema. Cannot be used with initialize_db.")
	...
	if parsed_args.initialize_db or parsed_args.update_db:
		from .db import Database
		db = Database()
	...
	del args["initialize_db"], args["update_db"]
